# A script to reformat Neale lab UK Biobank results (http://www.nealelab.is/uk-biobank)
...
def main():
	...
	with(open(args.outfile, 'w')) as outfile:
		...
		for line in infile:
			...
			snp_id, beta, se = splits[0], splits[7], splits[8]
			chrom, pos, major, minor = snp_id.split(':')
			snp_id = chrom + ':' + pos
			zscore = str(float(beta) / float(se))
			ordered_fields = [chrom, pos, snp_id, major, minor, zscore]
