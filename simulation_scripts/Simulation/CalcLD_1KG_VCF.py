...
def Read_Locus(file_name):
    ...
    all_positions = []
    all_data = []
    all_data.append(["chr", "rsID", "familyID", "pos", "effect", "alternative"])
    counter = 0
...
def Match_Ref_Panel_Alleles(vcf_rows, input_data, pos_header, effect_header, alt_header):
...
def main():
    ...
    drop_mono=True
    [all_positions, all_data] = Read_Locus(locus_name)
    ...
    [final_reference, flipped_data] = Match_Ref_Panel_Alleles(vcf_rows, filtered_data,   position, effect_allele, alt_allele)
